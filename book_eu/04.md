...
Funtzioak programen barruan dauden programa txikiak dira. Adibidez, funtzio horrek pantailan agur bat besterik ez du ateratzen:
...
Ikus daitekeenez, funtzio bat definitzeko `def`, 'hitza erabiltzen da, eta, ondoren, funtzioaren izena, kasu honetan, `agurra` eta `()` parametroen zerrenda, kasu honetan hutsik dagoena. Funtzioaren barruan, nahi ditugun aginduak jar ditzakegu.

Eta Pythonen estiloarekin jarraituz, funtzioaren barruan dagoen kodearen aurretik tabulazioa edo espazioak daudela ikusten da.
...
```python
agur("Neko") #pertsona aldagaia  "Neko" izango da
agur("Ada") #pertsona aldagaia "Ada" izango da
```
...
Idatz ezazu programa bat `karratua` izeneko funtzio batekin, eta parametro hau izango du: funtzioak emaitza biderkatu eta pantaila bakoitzean erakutsi behar du.
...
Aldiz, bigarren parametroa pasatzen ez badugu, `aldiz`ek lehenetsitako balioa hartuko du:
...
Python-en funtzioei esker, parametro kopuru zehaztugabea pasatzeko aukera daukazu. Arraro samarra dirudi, baina baliagarria da.
...
Erreparatzen baduzu, `balioak` parametroa zehaztu dugu, aurretik `*` bat jarrita. Horrekin esan nahi dizugu ez dela parametro bakarra, edozein luzera izan dezakeen zerrenda baizik.
...
Idatz ezazu programa bat parametro mugagabeak edo dinamikoak jasotzen dituen `hitzak_batu` izeneko funtzio batekin. Batzeko funtzioak parametroak hartu behar ditu, eta horiekin guztiekin testu bat itzuli, esaldi bat osatuz.
...
**"3aren araua"** ere aplika dezakezu. Zerbait errepikatu behar duzun hirugarren aldian, automatizatu egin behar duzu.
...
Gure funtzioak ondo zehaztuak baditugu, lana PIIIIILA aurreztuko dugu.
...
Berez, aditua zarenean, zure eginkizuna bera baino lehen testa idaztea da zure betebeharra.
...
- Funtzio batek ez luke kanpoan dagoen ezer aldatu behar. Ezustekorik izan nahi ez baduzu, funtzio batek ez luke programaren barruan ezustekorik behar.
...
2. `kantitateak`: zenbaki osoak, kantitatea adierazita.
3. `prezioak`: zenbaki hamartarrak, produktu bakoitzaren prezioa adierazita.

Funtzioak produktu bakoitza aztertu eta guztizko prezioa kalkulatu behar du, kantitatearen eta prezioaren arabera. Produktu bakoitzaren guztizko prezio hori erakutsi behar da, eta programaren amaieran, batura osoaren prezioa erakutsi behar da.
