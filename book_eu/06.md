...
Baliteke programazioan bikaina izatea. Baina, hala ere, zure programek huts egiten jarrai dezakete, gauza batzuk zure programaren kontroletik kanpo daudelako. Beraz, nahiz eta zure programa zuzena izan, gauzak oker joan daitezke.

Adibidez, zure programak erabiltzaileari zenbaki bat idaztea eskatzen badio, baina erabiltzaileak letrak idazten baditu edo ezer idazten ez badu, zure programak huts egingo du.
...
Edukia fitxategiaren amaieran gehitu nahi badugu, fitxategia `"a"` moduan ireki behar dugu:
...
Eta fitxategian erabiltzen dugu:
...
1. `def init__(self)`: aukeren zerrenda bat jasotzen du parametro gisa.
...
1. `def __init__ (self, fitxategia)`: json fitxategi baten izena hartzen du parametrotzat, eta haren edukia zerrenda batean kargatu behar da. Zerrenda hori atributu gisa definituko da. Edukiak hiztegi-zerrenda bat izan behar du, formato honekin `[{"id": 1, "izena": "Juan"}, {...}]
2. `def (self, izena)`: `True` ala `False` itzuli parametro gisa pasatzen den `izena` zerrendan badago.
3. `def xehetu (self)`: zerrendako izen guztiak letra xeheetara pasatu behar dira.
4. `def posizioa (self, nombre)`: izen hori dagoen posizioa itzuli behar duzu.
...
Gero, klasea erabili behar duzu beste fitxategi batean inportatuz.
...
Sor ezazu `Jokalaria` izeneko klase bat, eta eduki hau izango du:
...
Sor ezazu `Taldea` izeneko klase bat, eta eduki hau izango du:
