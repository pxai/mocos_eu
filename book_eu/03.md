...
Orain arte datu sinpleekin, zenbaki bat, testu bat eta abar duten aldagaiekin jolasten aritu gara. Baina badira datu konplexuagoak sortzeko aukera ematen diguten beste datu mota batzuk ere: datu soil bat baino zerbait gehiago eduki dezakete.
...
```python
zenbakiak = [1, 2, 3, 4, 5, 6, 7, 8, 9]
zenbakiak[: 6] # [1, 2, 3, 4, 5, 6] zenbakiak
```
...
Azken elementoa kentzeko, `pop` funtzioa ere erabil daiteke. Honek elementua kentzen du eta itzultzen du:
...
Indizea erabiliz ere egin daiteke zerrenda. Horretarako, `range` funtzioa erabili beharko dugu, eta zerrendaren luzera eman beharko diogu, `len' erabiliz:
...
Baina nola dakit zer egitura mota diseinatu behar dudan? Nola erabiliko duzun arabera. Batzuetan denak ibili beharko dituzu, beste batzuetan elementu jakin bat eskuratu beharko duzu... zure programak eskatzen duenaren arabera, egitura jakin bat diseinatu beharko duzu, zure programa azkarrago joan dadin.
...
Izan ere, ordenagailuarentzat, testu bat zerrenda bat bezalakoa da. Letra zerrenda (edo katea) honela tratatu dezakegu:
...
Idatz ezazu 5 elementuko zerrenda bat hasten duen programa bat:
- Zenbaki zerrenada (0ra hasitakoak).
- Eskuz hasitako 5 izeneko zerrenda.
...
Idatzi zerrenda bat kudeatzeko programa bat, erabiltzaileari 4 aukera dituen menu bat erakusteko: (1) elementua sartu, (2) ezabatu, (3) erakutsi eta (4) irten. Menua erabiltzaileak 4. aukera sartzen ez duen bitartean erakutsi behar da. 1. aukera aukeratuz gero, edozein balioko `append` egingo da, 2 aukeratuz gero, `pop` egingo da, eta 3 aukeratuz gero, zerrendako edukia erakutsiko da.
...
Zein ONA den profa Ada andereñoa
```
...
Sortu 10 zenbaki osoko zerrenda bat definitzen duen programa bat. Ondoren, sortu begizta bat zerrendan elementuren bat errepikatuta dagoen bilatzko. Errepikatutako bat aurkitzea nahikoa da.
...
Sortu 10 zenbaki osorekin hasitako zerrenda bat definitzen duen programa bat. Begizta batean, pantaila bidez erakusten ditu elementu guztiak. Gero, beste begizta bat sortzen du, eta, bertan, elementuak lekuz aldatzen aldatzen ditu, indizeetan `random` metodoa erabiliz. Gero emaitza erakusten du.
